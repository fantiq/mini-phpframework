...
{
	protected $dbh=null;
	protected $stmt=null;		// 预处理的返回class 
	...
	public function query($sql='',$method=null,$row=false)
	{
		...
		else
		{
			...
			return $row==false?$stmt->fetchAll():$stmt->fetch();
		}
	}
	/**
	 * 根据请求type组合生成SQL字符串
	...
	private function exec()
	{
		...
		$this->stmt = $this->dbh->prepare(self::$sql);
	...
	private function get_table_info($tablename='')
	{
		...
		return array('engine'=>self::$t_engines[$tablename],'rows'=>self::$t_rows[$tablename]);
	}
	//---------------------------------------------
	/**
	 * 生成tablename
	...
	public function connect()
	{
		...
		$dsn = config::get('db.driver','mysql').':host='.config::get('db.host','127.0.0.1').';port='.config::get('port',3306).';dbname='.$this->dbname.';charset='.config::get('db.charset').';';
		if(extension_loaded('PDO'))
		{
			$this->dbh=new PDO($dsn,config::get('db.user','root'),config::get('db.passwd',''));
		}
		else
		{
			$this->error('连接数据库错误');
	...
	public function sql()
	{
		return str_replace(array_keys($this->params), array_values($this->params), self::$sql);
	}
