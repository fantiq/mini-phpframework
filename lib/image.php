<?php
$img = new image();
$img->verify_code(60,6,array(255,255,255),array(0,201,87));
class image
{
	private $img=array(
		'filename'=>'',
	...
	public function __construct($path='')
	{
		if(extension_loaded('gd')&&!empty($path)&&!($image_info = getimagesize($path)))
		{
			...
			$gd = gd_info();
			$this->gd_version = preg_match('/2\.\d\.\d/i', $gd['GD Version'])?2:1;
		}
	}
	/**
	 * 验证码生成
	 ...
	 */
	public function verify_code($h=0,$n=3,$bgcolor=array(0,0,0),$color=array(0,0,0))
	{
		$this->image_new_source($h,$n,$bgcolor);
		$this->create_code($n,$color);
	 ...
	 * @return void
	 */
	private function image_new_source($h=0,$n=0,$color=array(0,0,0))
	{
		//创建图片
		$this->image['h']=$h;
		$this->image['w']=$n*$h/2;
		...
		// 填充颜色
		imagefill($this->image['im'], 0, 0, $this->get_color($this->image['im'],$color));
	}
	/**
	 * 存储生成的图片文件
	 ...
	 * @return [type]     [description]
	 */
	private function create_code($n=0,$color=array(0,0,0))
	{
		//生成随机数
		$code='';
		...
		//写入图片
		$font_size=$this->gd_version==2?$this->image['h']*2/11:$this->image['h'];
		$font_size=$this->image['h']*3/5; //1pt = 3/4 px
		$str_x = $this->image['h']*mt_rand(20,30)/100;
		$str_y = $this->image['h']*4/5;
		$font_path = dirname(__FILE__).DIRECTORY_SEPARATOR.'fonts'.DIRECTORY_SEPARATOR.'roughtrad.ttf';
		$im_color = $this->get_color($this->image['im'],$color);
		for($i=0;$i<$n;$i++)
		{
			$angle=mt_rand(0,15);
			$pos = imagettftext($this->image['im'], $font_size, $angle, $str_x, $str_y, $im_color, $font_path, $code[$i]);
			$str_x=$pos[4]*mt_rand(88,100)/100;
		}
		// set session
	}
	/**
	 ...
	 * @return void
	 */
	private function interference($color=array(0,0,0),$mode=1)
	{
		$im_color = $this->get_color($this->image['im'],$color);
		for($i=0;$i<2;$i++)
		{
	 ...
	 * @return void
	 */
	private function get_color($im=null,$color=array(0,0,0))
	{
		list($r,$g,$b) = $color;
